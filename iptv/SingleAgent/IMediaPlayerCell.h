#ifndef IMEDIAPLAYERSERVERCELL_H
#define IMEDIAPLAYERSERVERCELL_H
...
class IMediaPlayerCell : public IMediaPlayer {
public:
    ...
    virtual void removeMediaByIndex(long);
    ...
    virtual int getAudioPIDs(char* audioPIDs);
    virtual void setAudioPID(long audioPID);
    virtual long getSubtitlePID();
    virtual int getSubtitlePIDs(char* subtitlePIDs);
...
protected:
    IMediaPlayerCell();
    IMediaPlayerCell(const IMediaPlayerCell&);
    IMediaPlayerCell& operator = (const IMediaPlayerCell&);

    Player* m_player;

    friend IMediaPlayer* createIMediaPlayer(const char*);
};

#endif //IMEDIAPLAYERSERVER_H
